...
export function NotesList({ notes = [], onNoteSelect, onDeleteNote }) {
  const [copiedNoteId, setCopiedNoteId] = useState(null);
        ...
        <div className="space-y-8 p-4 py-8 z-10 relative">
          {sortedNotes.map(note => (
            ...
            >
              ...
              {/* Delete Button - Position in top right next to copy button */}
              <button
                onClick={(e) => {
                  e.stopPropagation();
                ...
                className="absolute top-1 right-8 p-1.5 rounded-md transition-all flex items-center gap-1 
